...
import { COMPLIANCE_ATTACHMENT_TYPES, COMPLIANCE_REQUIREMENTS, COMPLIANCE_STATUSES, complianceDocumentView, complianceIndicator, compliancePayload, complianceSummary, filterComplianceRows, normalizeComplianceEditor } from '../lib/compliance'
...
function ComplianceRow({ row, onEdit, onUpload, showLicense, showCoa, showBond, uploadType }) {
  const marker = complianceIndicator(row)
  const uploadLabel = uploadType === 'certificate_of_authority' ? 'COA' : uploadType === 'bond' ? 'Bond' : 'License'
  const markerStyles = {
...
function ComplianceEditor({ row, onClose, onSave, saving, error, initialUploadType = 'license' }) {
