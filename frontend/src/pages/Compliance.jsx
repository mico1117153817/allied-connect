    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-7xl mx-auto px-4 py-6 space-y-6">
        ...
        <section className="bg-white rounded-xl shadow-sm p-5">
          ...
          {!isLoading && !isError && <div className="overflow-x-auto"><table className="w-full min-w-[1050px] text-sm"><thead><tr className="border-b text-left text-gray-500"><th className="p-3">State</th><th className="p-3">Overall</th><th className="p-3">Confidence</th><th className="p-3">License</th><th className="p-3">COA</th><th className="p-3">Bond</th><th className="p-3">State Portal</th><th className="p-3">Issues</th><th className="p-3">Action</th></tr></thead><tbody>{visibleRows.map(row => <ComplianceRow key={row.state} row={row} onEdit={() => { setSelectedUploadType('license'); setSelected(row) }} onUpload={type => { setSelectedUploadType(type); setSelected(row) }} />)}</tbody></table>{visibleRows.length === 0 && <p className="text-gray-500 text-center py-8">No states match the current filters.</p>}</div>}
        </section>
      </main>
...
function ComplianceRow({ row, onEdit, onUpload }) {
