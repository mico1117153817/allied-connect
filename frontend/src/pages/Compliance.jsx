...
function ComplianceRow({ row, onEdit, onUpload, showLicense, showCoa, showBond, showAnnualReport, uploadType }) {
  const marker = complianceIndicator(row)
  const uploadLabel = uploadType === 'certificate_of_authority' ? 'COA' : uploadType === 'bond' ? 'Bond' : uploadType === 'annual_report' ? 'Annual Report' : 'License'
  const markerStyles = {
